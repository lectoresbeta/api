...
**Implementación:** `PARTIAL` (2026-09-26). El listado existe: `GET /api/v1/me/claims` devuelve las propias, de la
...
~~El hilo con el moderador ([`FEAT-MOD-009`](FEAT-MOD-009-moderator-conversation.md))~~ —
**existe** (revisado el 2026-09-26): `GET` y `POST /api/v1/me/claims/{claimId}/messages`.
[`FEAT-MOD-002`](FEAT-MOD-002-review-claim.md) también está implementada, así que las
reclamaciones ya se resuelven.

**Falta** lo que la lista no cuenta: el **detalle de una reclamación**, y **la resolución con
su motivo**. Hoy `GET /api/v1/me/claims` devuelve el estado y nada más, así que quien denunció
ve que su reclamación se cerró pero no por qué — que es justo lo que esta ficha existe para
evitar.
